...
export class SizeFieldComponent implements OnInit {
  ...
  set _field(field: SizeField) {
    ...
    if (this.field.isDisabled) {
      this.width.disable({emitEvent: false});
  ...
  public toggleRelative(): void {
    ...
    if (this.isRelative) {
      value = {width: 0.5, height: 0.5};
      this.width.setValue(value.width * 100);
      this.height.setValue(value.height * 100);
      this.min = 0;
      this.max = 100;
      this.step = 10;
    } else {
      value = this.field.defaultValue ?? {width: 0, height: 0};
  ...
  protected onFieldValueChanged(value: unknown): void {
  ...
  protected onFieldStateChanged(isDisabled: boolean): void {
    if (isDisabled) {
      ...
      this.width.enable({emitEvent: false});
      this.height.enable({emitEvent: false});
    }
  }
