...
import type {Color, Vector2} from "../models/math";
...
import {FIGVerticalSliderType, type FIGVerticalSliderWidget} from "../models/widgets/vertical-slider.widget";
...
export class FIGLuaSol2Formatter extends FIGFormatter {
  ...
  private formatSize(label: string, type: FIGWidgetType, size: SizeField): string[] {
  ...
  protected override formatTabItem(widget: FIGTabItemWidget): void {
    ...
    if (varFlags.length === 0) {
      ...
      this.append(
        `${varOpen}, ${varSelected} = ImGui.BeginTabItem(${this.formatString(widget.label)}, ${varOpen}${varFlags})`,
      );
  ...
  protected override formatText(widget: FIGTextWidget): void {
    ...
    if (!widget.hasBullet && !widget.isDisabled && !widget.color && !widget.isWrapped) {
      ...
      if (widget.isDisabled) {
        ...
        this.append("ImGui.PopStyleColor()");
        this.append(
          `ImGui.TextColored(${FIGLuaSol2Formatter.formatColor(widget.color)}, ${this.formatString(widget.text)})`,
        );
      } else {
  ...
  protected override formatButton(widget: FIGButtonWidget): void {
    if (widget.isSmall) {
      ...
      this.append(
        `if ImGui.ArrowButton(${this.formatString(widget.label)}, ${FIGLuaSol2Formatter.formatDir(widget.arrow)}) then`,
      );
    } else {
      let size: string = "";
  ...
  protected override formatInputText(widget: FIGInputTextWidget): void {
    ...
    if (!widget.hint) {
      this.append(
        `${varDef} = ImGui.InputText(${this.formatString(widget.label)}, ${varText}, ${widget.bufferSize}${varFlags})`,
      );
    } else {
      this.append(
        `${varDef} = ImGui.InputTextWithHint(${this.formatString(widget.label)}, ${this.formatString(widget.hint)}, ${varText}, ${widget.bufferSize}${varFlags})`,
      );
    }
    this.formatTooltip(widget);
  }
  ...
  protected override formatInputTextarea(widget: FIGInputTextareaWidget): void {
    ...
    this.append(
      `${varDef} = ImGui.InputTextMultiline(${this.formatString(widget.label)}, ${varText}, ${widget.bufferSize}, ${varSize}${varFlags})`,
    );
    this.formatTooltip(widget);
  }
  ...
  protected override formatInputColorEdit(widget: FIGInputColorEditWidget): void {
    ...
    if (widget.flags !== 0 && widget.flags !== FIGInputColorEditMasks.DefaultOptions_) {
      varFlags = this.formatFlags<FIGInputColorEditFlags>(
        widget.flags,
        FIGInputColorEditWidget.flags,
        FIGInputColorEditFlags,
        "ImGuiColorEditFlags",
      );
    }
  ...
  protected override formatListBox(widget: FIGListBoxWidget): void {
    ...
    this.append(
      `${varCurrentItem} = ImGui.ListBox(${this.formatString(widget.label)}, ${varCurrentItem}, ${varItems}, #${varItems})`,
    );
    this.formatTooltip(widget);
  }
  ...
  protected override formatRadio(widget: FIGRadioWidget): void {
    const group: FIGRadioWidget[] = widget.parent!.filter<FIGRadioWidget>((item) =>
      FIGRadioWidget.filterByGroupId(item, widget.groupId),
    );
  ...
  protected override formatCombo(widget: FIGComboWidget): void {
    ...
    this.append(
      `${varCurrentItem} = ImGui.Combo(${this.formatString(widget.label)}, ${varCurrentItem}, ${varItems}, #${varItems})`,
    );
    this.formatTooltip(widget);
  }
