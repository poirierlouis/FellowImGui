...
interface FIGWidgetCtor {
  new (options: unknown): FIGWidget;
}

type FIGDictionary = Record<string, unknown>;
...
export class FIGWidgetFactory {
  ...
  public static create(type: FIGWidgetType): FIGWidget | undefined {
    const builder: FIGWidgetBuilder | undefined = FIGWidgetFactory.findBuilder(type);
  ...
  public static clone(widget: FIGWidget, parent?: FIGContainer): FIGWidget {
    const prototype: {constructor: FIGWidgetCtor} = Object.getPrototypeOf(widget);
    const options: unknown = FIGWidgetFactory.getOptions(widget);
  ...
  public static filterBetween(from: FIGWidgetType, to?: FIGWidgetType): FIGWidgetBuilder[] {
    ...
    for (const builder of FIGWidgetFactory.builders) {
  ...
  private static findBuilder(type: FIGWidgetType): FIGWidgetBuilder | undefined {
    return FIGWidgetFactory.builders.find((item) => item.type === type);
  }

  private static getOptions(widget: FIGWidget): unknown {
    const keys: string[] = Object.keys(widget).filter((key) => !FIGWidget.excludeKeys.includes(key));
    const options: FIGDictionary = {};

    for (const key of keys) {
      const value: FIGDictionary | unknown = (widget as unknown as FIGDictionary)[key];

      if (Array.isArray(value)) {
        const innerValue: unknown[] = [];
        ...
        const innerValue: FIGDictionary = {};

        for (const innerKey of Object.keys(value)) {
          innerValue[innerKey] = (value as FIGDictionary)[innerKey];
        }
        options[key] = innerValue;
      } else {
        options[key] = value;
